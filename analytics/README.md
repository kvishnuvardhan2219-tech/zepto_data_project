...
I converted age and fare into z-scores just to prove i could do it and that it works - before standardizing, age had mean 29.32/std 12.98 and fare has mean 32.10/std 49.70. After standardizing, both has mean ~0 and ~1, which is supposed to happen. That wasn't used anywhere else, just a quick sanity check.
...
SMOTE ended up being the best pick since it had highest F1 score - it balances precision and recall better than the other 2 options. The baseline was more precise but missed more survivors, and class_weight caught more survivors but had more false alarms.
...
Looking at the residual plot, there's a clear funnel shape - errors stay small for low predicted fares but get a lot bigger for higher predicted fares, including one residual over 400. This means the model shows **heteroscedasticity** - it's not equally accurate across all fare ranges. That tracks with fare being right-skewed with some pricey outlier tickets that are hard to predict exactly.
...
The final trained pipeline is saved as `titanic_full_pipeline.joblib`. I tested reloading it and running it on raw, un-preprocessed data, and it predicted correctly = so it's ready to be reused without needing to rebuild any of the preprocessing steps.
